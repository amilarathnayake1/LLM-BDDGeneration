...
class EvaluationConfig:
    """Configuration for the GPT-4 evaluator"""
...
class GPT4BDDEvaluator:
    """
    LLM-based BDD scenario evaluator using GPT-4 model.
...
{{
    ...
    def _setup_client(self):
        """Initialize the OpenAI client for GPT-4 API"""
        try:
            self.client = openai.OpenAI(
                api_key=self.config.api_key
            )
            logger.info("GPT-4 client initialized successfully")
    ...
    def _make_api_call(self, prompt: str) -> Dict:
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert BDD scenario evaluator. Always respond with valid JSON containing 'rating' and 'reasoning' fields."},
            ...
            if not all(key in result for key in ['rating', 'reasoning']):
                raise ValueError("Invalid response structure from GPT-4")
    ...
    def evaluate_single_scenario(self, 
        ...
        try:
            ...
            return EvaluationResult(
                ...
                reference_scenario=reference_scenario,
                ai_generated_scenario=test_scenario,
                gpt4_rating=result['rating'],
                gpt4_reasoning=result['reasoning'],
            ...
            return EvaluationResult(
                ...
                reference_scenario=reference_scenario,
                ai_generated_scenario=test_scenario,
                gpt4_rating=-1,  # Error indicator
                gpt4_reasoning=f"Evaluation failed: {str(e)}",
                evaluation_timestamp=pd.Timestamp.now().isoformat()
            )
    ...
    def evaluate_batch(self, data_df: pd.DataFrame, 
        ...
        for idx, row in data_to_evaluate.iterrows():
            try:
                ...
                # Debug: Check result
                logger.debug(f"Evaluation result: rating={result.gpt4_rating}")
    ...
    def _results_to_dataframe(self, results: List[EvaluationResult]) -> pd.DataFrame:
        ...
        for result in results:
            data.append({
                ...
                'LLM Evaluator Rating': result.gpt4_rating
            })
    ...
    def _results_to_detailed_dataframe(self, results: List[EvaluationResult]) -> pd.DataFrame:
        ...
        for result in results:
            data.append({
                ...
                'BDD-AI Generated': result.ai_generated_scenario,
                'LLM Evaluator Rating': result.gpt4_rating,
                'LLM Evaluator Reasoning': result.gpt4_reasoning,
