...
class ROUGEComparator(BaseComparator):
    def __init__(self, config: ROUGEConfig):
        """Initialize the ROUGE comparator with configuration"""
        self.config = config
    ...
    def compare_scenarios(self, 
        ...
        """Compare AI-generated and manual scenarios using ROUGE-L score"""
        ...
        # Calculate ROUGE-L score for each step pair
        step_scores = []
        for ai_step, manual_step in zip(ai_steps, manual_steps):
            ...
            scores = self.scorer.score(manual_text, ai_text)
            step_scores.append(scores['rougeL'].fmeasure)
    ...
    def _print_comparison_details(self, ai_steps: List[str], manual_steps: List[str]) -> None:
        ...
        print("\nStep Comparison Details (ROUGE-L):")
        print("-" * 80)
        for i, (ai, manual) in enumerate(zip(ai_steps, manual_steps)):
            ...
            print(f"Score  : {rouge_l_score * 100:.2f}%")