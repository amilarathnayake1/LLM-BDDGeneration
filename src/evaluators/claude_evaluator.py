...
class EvaluationConfig:
    """Configuration for the Claude evaluator"""
...
class ClaudeBDDEvaluator:
    """
    LLM-based BDD scenario evaluator using Claude (Anthropic) model.
    
    Implements the evaluation framework described in the research project
    for rating AI-generated BDD scenarios against reference scenarios.
    """
    ...
    def __init__(self, config: EvaluationConfig):
        ...
        self.config.model_name = ClaudeClientFactory.get_model_name(self.config.model_name)
        
        self.evaluation_prompt = """ You are an expert software testing evaluator specializing in Behavior-Driven Development (BDD) scenarios. Your task is to
evaluate the BDD scenario against the provided requirements(both user story and description) and rate it on a scale from 1 to 5, where
...
**Response Format:**
Please respond in the following JSON format (IMPORTANT: Keep the reasoning on a single line without line breaks):
{{
    "rating": <1, 2, 3, 4, or 5>,
    "reasoning": "Your detailed explanation on a single line"
}}"""
    ...
    def _setup_client(self):
        ...
        try:
            ...
            logger.info(f"Claude client initialized successfully (provider: {provider})")
    ...
    def _make_api_call(self, prompt: str) -> Dict:
        ...
        try:
            message = self.client.messages.create(
                ...
                system="You are an expert BDD scenario evaluator. Always respond with valid JSON containing 'rating' and 'reasoning' fields. Keep the reasoning text on a single line without newlines.",
                messages=[
            ...
            # Fix: Replace literal newlines in the reasoning field with spaces
            # This handles cases where Claude includes newlines in the JSON string
            try:
                # First attempt: parse as-is
                result = json.loads(content)
            except json.JSONDecodeError:
                # If that fails, try to fix newlines within string values
                # Replace newlines that are within quotes with spaces
                import re
                
                # Fix newlines between quotes in the JSON
                fixed_content = re.sub(
                    r'"reasoning"\s*:\s*"([^"]*(?:\\.[^"]*)*)"',
                    lambda m: '"reasoning": "' + m.group(1).replace('\n', ' ').replace('\r', ' ').replace('  ', ' ') + '"',
                    content,
                    flags=re.DOTALL
                )
                ...
                try:
                    ...
                    # Last resort: try to extract rating and reasoning manually
            ...
            # Clean up the reasoning text (replace multiple spaces with single space)
            result['reasoning'] = ' '.join(result['reasoning'].split())
    ...
    def evaluate_single_scenario(self, 
        ...
        try:
            ...
            return EvaluationResult(
                ...
                reference_scenario=reference_scenario,
                ai_generated_scenario=test_scenario,
                claude_rating=-1,  # Error indicator
                claude_reasoning=f"Evaluation failed: {str(e)}",
                evaluation_timestamp=pd.Timestamp.now().isoformat()
            )
    ...
    def evaluate_batch(self, data_df: pd.DataFrame, 
        ...
        for idx, row in data_to_evaluate.iterrows():
            try:
                ...
                logger.debug(f"Row index: {idx}")
                logger.debug(f"Row columns: {list(row.index)}")
                
                # Check if required columns exist
                required_cols = ['User Story', 'Description', 'BDD-Reference', 'BDD-AI Generated', '#']
                missing_cols = [col for col in required_cols if col not in row.index]
                if missing_cols:
                    logger.error(f"Row {idx} missing columns: {missing_cols}")
                    logger.error(f"Available columns: {list(row.index)}")
                    continue
                
                # Check for empty values
                if pd.isna(row['User Story']) or pd.isna(row['Description']) or pd.isna(row['BDD-AI Generated']):
                    logger.warning(f"Row {idx} has empty required fields, skipping")
                    continue
                
                logger.debug(f"User Story: {str(row['User Story'])[:50]}...")
                logger.debug(f"Description: {str(row['Description'])[:50]}...")
                ...
                # Debug: Check result
                logger.debug(f"Evaluation result: rating={result.claude_rating}")
    ...
    def _results_to_dataframe(self, results: List[EvaluationResult]) -> pd.DataFrame:
        ...
        for result in results:
            data.append({
                ...
                'LLM Evaluator Rating': result.claude_rating
            })
    ...
    def _results_to_detailed_dataframe(self, results: List[EvaluationResult]) -> pd.DataFrame:
        ...
        for result in results:
            data.append({
                ...
                'BDD-AI Generated': result.ai_generated_scenario,
                'LLM Evaluator Rating': result.claude_rating,
                'LLM Evaluator Reasoning': result.claude_reasoning,
