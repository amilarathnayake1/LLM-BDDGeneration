...
from config import USECSConfig

class USECSComparator(BaseComparator):
    def __init__(self, config: USECSConfig):
        """Initialize the Universal Sentence Encoder style comparator using SentenceTransformer"""
        self.config = config
        # Using a multilingual model that's similar to USE
    ...
    def compare_scenarios(self, 
        ...
        try:
            ...
            raise Exception(f"Error comparing scenarios: {e}")

    def _print_comparison_details(self, ai_steps: List[str], manual_steps: List[str]) -> None:
        """Print detailed comparison information"""
        print("\nStep Comparison Details (Universal Sentence Encoder Style + Cosine Similarity):")
        print("-" * 80)
